...
fun RegisterScreen(
    ...
    LaunchedEffect(state.isRegisteredIn) {
        if (state.isRegisteredIn) {
            navController.navigate(Routes.LoginScreen.routes) {
                popUpTo(Routes.RegisterScreen.routes) { inclusive = true }
            }
        }
    }
    ...
    Column(
        ...
        TextButton(
            onClick = { navController.navigate(Routes.LoginScreen.routes) },
            enabled = !state.isLoading
        ) {
            Text("Already have an account? Login", color = mainColor)
        }
    }
}